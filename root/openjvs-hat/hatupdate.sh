...
if ! grep -Fxq "dtoverlay=uart2-pi5" /boot/config.txt; then
   echo -e "\n" | sudo tee -a /boot/config.txt
   echo "dtoverlay=uart2-pi5" | sudo tee -a /boot/config.txt
   echo "dtoverlay=uart3-pi5" | sudo tee -a /boot/config.txt
else
   echo "Entries already found!"
fi
   echo "Copying OpenJVS config file"
...
sudo cp /root/openjvs-hat/cmdline.txt /boot/cmdline.txt
