...
if [ $openjvshat -eq 0 ]; then
    ...
    if [ "$version" -eq 4 ]; then
        ...
        # Search for 'DEVICE_PATH /dev/ttyAMA3' in openjvs config
	...
	if [ -n "$uart3_found" ] && [ -n "$uart4_found" ]; then
        ...
        # Search for 'DEVICE_PATH /dev/ttyAMA2' in openjvs config
	...
	if [ -n "$uart2_found" ] && [ -n "$uart3_found" ]; then
        ...
        echo "$(date +"%Y%m%d %H%M%S") Raspberri pi 4 or 5 not detected, no action needed."
    fi
else
    echo "$(date +"%Y%m%d %H%M%S") OpenJVS is not enabled."
fi
