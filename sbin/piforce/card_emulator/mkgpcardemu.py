        ...
        while nocard is True:
            if glob.glob(DropFolderGlobPath):
                ...
                with open(CardFileName, "rb") as in_file: # Open original card file and read contents
                    ...
                    if len(CardBytes) == 77:
                        print('---> Card data is correct length for Mario Kart')
                        q.put(CardBytes) # Send card contents to the cardpoll queue
                    else:
                        print('---> Card data invalid length for Mario Kart')
...
def CardTranslation(TriforceString,CardFileName,Mode):
    CARDString = TriforceString[TriforceString.find("02 4E 53 00 00 00 30 31 30")+27: TriforceString.find("02 4E 53 00 00 00 30 31 30") +27 + 207]
    CardCRC = 0
...
def main():
    ...
    if not args.mode:
        parser.error('Please specify an ID mode! Example: -m -m mkgp or -m mkgp2')
        sys.exit(0)
                ...
                while comport['buffer']:
                    ...
                    if len(ReadInput)>9:
                        ...
                        if CurrentCommand == CARD_WRITE_TEXT:
                            ...
                            if (rawfilename != ""):
                                cp = subprocess.run(["python3", "/sbin/piforce/card_emulator/mkgp_card_data.py", "-m", mkgpmode, "-f", rawfilename])
                                rawfilename = ""
                    ...
                    if CurrentCommand == CARD_WRITE and "05" in ReadInput:
                        ...
                        CardBytes, CardFileName = CardTranslation(PriorCommand,CardFileName,mkgpmode)
                        print (CardBytes)
                    ...
                    if CurrentCommand == CARD_80 and "05" in ReadInput:
                        ...
                        comport['ser'].write(output)
                        PrintDataFile = "/var/www/html/cards/"+mkgpmode+"/"+os.path.basename(CardFileName)+".printdata.php"
