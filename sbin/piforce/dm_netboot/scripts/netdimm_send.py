...
def main() -> int:
    ...
    logging.basicConfig(filename="/var/log/netdimm_send.log", format='%(asctime)s %(message)s', filemode='w')
    ...
    p = subprocess.Popen(["pigz", "-cd", "-p", "8", args.image], stdout = subprocess.PIPE, stderr=None, close_fds=True)
    with io_method(p.communicate()[0]) as fp:
        data = FileBytes(fp)
