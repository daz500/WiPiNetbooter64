...
def main() -> int:
    ...
    pidpath = '/sbin/piforce/dm_netboot/scripts/serverpid.txt'
    print(pidpath)
    if exists(pidpath):
        print('exists')
    ...
    pidwrite = 'sudo echo -n '+str(currentpid)+' | tee '+pidpath
    os.system(pidwrite)

    args = parser.parse_args()

    logging.basicConfig(filename="/var/log/netdimm_ensure.log", format='%(asctime)s %(message)s', filemode='w')
