    ...
    if isinstance(data, dict):
        if ip in data:
            ...
            if isinstance(data, dict):
                ...
                if 'sram_save' in data:
                    settings.sram_save = bool(data['sram_save'])
...
def main() -> int:
    ...
    logging.basicConfig(filename="/var/log/netdimm_menu.log", format='%(asctime)s %(message)s', filemode='w')
    logger=logging.getLogger()
    logger.setLevel(logging.DEBUG)
    sys.stderr.write = logger.error
    sys.stdout.write = logger.info

    # Check if OpenJVS is in use
    with open('/sbin/piforce/openmode.txt') as openfile:
        openmode = openfile.read()
        print("OpenJVS file has been checked")
        print("OpenJVS mode is", openmode)

    # Check if time hack is enabled
    with open('/sbin/piforce/zeromode.txt') as zerofile:
        zeromode = zerofile.read()
        print("Time hack file has been checked")
        print("Time hack mode is", zeromode)

    # Grab the last played game for SRAM and EEPROM
    with open('/var/www/logs/log.txt') as f:
        lastplayedline = f.readline().split()
        lastplayed = lastplayedline[0]
        sramsavefile = '/sbin/piforce/dm_netboot/srams/'+lastplayed+'.sram'
    ...
    while True:
        ...
        games: List[Tuple[str, str, bytes, str]] = []
        romdir = os.path.abspath(args.romdir)
        success: bool = True

        with open('/var/www/html/csv/dimms.csv', newline='') as vertfile:
            vertreader = csv.reader(vertfile, delimiter=',')
            for vertrow in vertreader:
                if vertrow[1] == args.ip:
                    if (vertrow[7] == 'vertical'):
                        vertical = True
                        print('Game selection is vertical')
                    else:
                        vertical = False
                        print('Game selection is horizontal')

        filedict = {}
        with open('/var/www/html/csv/romsinfo.csv', newline='') as csvfile:
            filereader = csv.reader(csvfile, delimiter=',')
            for row in filereader:
                filedict[row[1]] = row[4], row[16], row[14], row[0], row[12], row[10]

        for filename in [f for f in os.listdir(romdir) if os.path.isfile(os.path.join(romdir, f))]:
            # Grab the header so we can parse it.
            fullname = os.path.join(romdir, filename)
            #print(filedict)
            if vertical:
                for key, value in filedict.items():
                    if filename == key and ('Naomi' in value[3] or 'Atomiswave' in value[3]) and value[4] == "Yes" and value[5] == "Vertical":
                        games.append((fullname, value[0].replace("(","").replace(")",""), "{:<4}".format(value[1]).encode(), value[2]))

            else:
                for key, value in filedict.items():
                    if filename == key and ('Naomi' in value[3] or 'Atomiswave' in value[3]) and value[4] == "Yes" and value[5] == "Horizontal":
                        games.append((fullname, value[0].replace("(","").replace(")",""), "{:<4}".format(value[1]).encode(), value[2]))
        ...
        last_game_id: int = 0
        gamesconfig = b""
        for index, (filename, name, serial, openjvs) in enumerate(games):
        ...
        config = struct.pack(
            ...
            1 if settings.sram_save else 0,
        ...
        if success:
            with open(args.exe, "rb") as fp:
                ...
                with open('/var/log/menudump.bin', "wb") as dumpfile:
                    menudata.write_changes(dumpfile)
            ...
            try:
                ...
                with netdimm.connection():
                    while True:
                        ...
                        if msg:
                            if msg.id == MESSAGE_SRAM_READ:
                                print('SRAM message received')
                                if len(msg.data) != 0x8000:
                                    print("Got wrong size for SRAM!")
                                    return 1
                                else:
                                    with open(sramsavefile, "wb") as fp:
                                        fp.write(msg.data)
                                        print('Written SRAM file')
                            ...
                            if msg.id == MESSAGE_SELECTION:
                                ...
                                time.sleep(1.5)
                                send_message(netdimm, Message(MESSAGE_UNPACK_PROGRESS))

                                sramfile = '/sbin/piforce/dm_netboot/srams/'+os.path.basename(filename)+'.sram'
                                if os.path.exists(sramfile) and settings.sram_save:
                                    print("Restoring SRAM...")
                                    with open(sramfile, "rb") as fp:
                                        data = fp.read()
                                    send_message(netdimm, Message(MESSAGE_SRAM_WRITE, data), verbose=verbose)
                                else:
                                    print("SRAM backup disabled or no SRAM file found...")

                                # First, grab a handle to the data itself.
                                start = time.time()
                                io_method = io.BytesIO
                                print(f"Start unzipping ROM")
                                p = subprocess.Popen(["pigz", "-cd", "-p", "8", filename], stdout = subprocess.PIPE, stderr=None, close_fds=True)
                                fp = io_method(p.communicate()[0])
                                assert p.returncode == 0
                                print(f"ROM loaded into memory")
                                end = time.time()
                                print(end - start)
                                ...
                                if (openmode == 'openon'):
                                    openjvsmapping = games[index][3]
                                    dimmdict = {}
                                    with open('/var/www/html/csv/dimms.csv', newline='') as csvfile:
                                        filereader = csv.reader(csvfile, delimiter=',')
                                        for row in filereader:
                                            dimmdict[row[1]] = row[3]
                                        if (dimmdict[args.ip] == 'on'):
                                            openjvsCommand1 = 'killall -9 openjvs'
                                            os.system(openjvsCommand1)
                                            print("Starting OpenJVS", openjvsmapping)
                                            cp = subprocess.Popen(['openjvs', openjvsmapping], preexec_fn=os.setsid)
                                if (zeromode == 'hackon'):
                                    print("Starting Time Hack script", args.ip)
                                    cp = subprocess.Popen(['python3', '/sbin/piforce/timehack.py', args.ip], preexec_fn=os.setsid)
                                logCommand = 'sudo echo -n '+os.path.basename(filename)+' '+args.ip+' | tee /var/www/logs/log.txt'
                                ...
                                if len(msg.data) == SETTINGS_SIZE:
                                    (
                                        ...
                                        sramsetting,
                                        *rest,
                                    ) = struct.unpack("<IIIIIIIIIBBBBBBBBBBBB", msg.data[:48])
                                    ...
                                    settings.sram_save = sramsetting != 0
