...
class NetDimm:
    ...
    def set_time_hack(self, minutes: int) -> None:
    ...
    def __upload_file(self, data: Union[bytes, FileBytes], key: Optional[bytes], progress_callback: Optional[Callable[[int, int], None]]) -> None:
        ...
        progressfilename = "/var/log/progress_"+self.ip
        ...
        last = -1
        while addr < total:
            percentage = int(float(addr * 100) / float(total))
            self.__print("%08x %d%%\r" % (addr, percentage), newline=False)
            if last < percentage:
                last += 1
                progressfile.write(str(percentage) + '\n')
                progressfile.flush()
            if progress_callback:
